...
class SearchBar extends Component {
  ...
  getSearchResults = () => {
    ...
    return clientsFiltered.slice(0,5).concat(jobsFiltered.slice(0,5)).slice(0,5);
  };
  ...
  render() {
    ...
    return (
      <form className={styles.wrapper} onSubmit = {this.returnFirstSearchResult} onBlur = {this.handleBlur} onFocus = {this.handleFocus}>
        <input
          type="text"
      ...
      {searchString.length > 0 && show && (
        <SearchResultsDropdown {...this.props} searchResults = {searchResults} resetField = {this.resetField}/>
        )}
        </form>
